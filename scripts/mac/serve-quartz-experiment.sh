...
#   port  default: 8081 (static) or 8080 (dev)
#   host  default: localhost  (use 0.0.0.0 for LAN/Tailscale access)
#
# Examples:
#   bash serve-quartz-experiment.sh                        # static, localhost:8081
#   bash serve-quartz-experiment.sh static 8081 0.0.0.0   # static, LAN-accessible
#   bash serve-quartz-experiment.sh dev                    # dev server, localhost:8080
#   bash serve-quartz-experiment.sh dev 8080 0.0.0.0       # dev server, LAN-accessible
#
# Mode differences:
#   static — serves quartz-site/public with python3 http.server (recommended for ops)
#   dev    — runs npx quartz build --serve (live-reload, dev only)
...
if [ "$MODE" = "static" ]; then
    ...
    _print_urls
    echo ""
    exec python3 -m http.server "$PORT" --bind "$HOST" --directory "$OUTPUT_DIR"
else
    # dev mode
    _print_urls
    echo ""
